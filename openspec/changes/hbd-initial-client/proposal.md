...
| Securities | `get_securities(board, settlement)` | `broker.online.snapshot.get_securities(board, settlement)` | ✅ Tier 1 |
| Options | `get_options()` | `broker.online.snapshot.get_options()` | ✅ Tier 1 |
| Repos | `get_repos()` | `broker.online.snapshot.get_repos()` | ✅ Tier 1 |
| Portfolio | `get_personal_portfolio()` | `broker.online.snapshot.get_personal_portfolio()` | ✅ Tier 1 |
| Order book | `get_order_book(symbol, settl)` | `broker.online.snapshot.get_order_book(symbol, settl)` | ✅ Tier 1 |
| Snapshot | `get_market_snapshot()` | `broker.online.snapshot.get_market_snapshot()` | ✅ Tier 1 |
| Daily hist | `get_daily_history(sym, f, t)` | `get_daily_history(sym, f, t)` | ✅ Tier 1 |
| Intraday | `get_intraday_history(...)` | — | ❌ Deferred (Tier 2) |
| Orders | `hb.orders.*` | — | ❌ Deferred (Tier 2) |
| Real-time | `subscribe_*` | `broker.online.get_securities(...)  ` (Tier 3) | ❌ Deferred (Tier 3) |
...
- `market-data-polling`: 6 boards + options + repos + indices/MERVAL + personal portfolio + Level 2 order book via HTTP polling under `broker.online.snapshot.*` to `/Prices/*` endpoints, normalized DataFrames
- `historical-data`: daily history only from `/HistoricoPrecios/history` (intraday deferred to Tier 2)
- `common-infrastructure`: all 17 broker configs + `get_broker()` lookup, 4-type exception hierarchy, DataFrame numeric normalization helpers
